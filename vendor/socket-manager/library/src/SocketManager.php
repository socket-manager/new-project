...
class SocketManager
{
    ...
    public function cycleDriven(int $p_cycle_interval = 2000, int $p_alive_interval = 0): bool
    {
        ...
        foreach($dess as $cid => $des)
        {
            ...
            if($alive_check === 1)  // アライブチェック開始までのタイムアウトを検査
            {
                ...
                if($dif > $p_alive_interval)
                {
                    $this->logWriter('debug', [__METHOD__ => "[{$cid}]".LogMessageEnum::ALIVE_CHECK_START_TIMEOUT->message($this->lang), 'difference' => $dif, 'interval' => $p_alive_interval]);
    ...
    public function connect(string $p_host, int $p_port, bool $p_udp = false, int $p_retry = 0, int $p_interval = 1000): bool
    {
        ...
        for($i = 0; $i < $max; )
        {
            if($p_udp === true)
            {
                ...
                $this->logWriter('debug', [__METHOD__ => 'recv complete', 'recv data' => $buf, 'host' => $from, 'port' => $port]);
        ...
        if($p_udp === true)
        {
            ...
            $this->logWriter('debug', [__METHOD__ => 'udp connect', 'connection id' => $des['connection_id']]);
    ...
    private function select($p_utimer = 0): bool
    {
        ...
        if($flg_connect === 2)
        {
            ...
            $this->logWriter('debug', ['udp first recv data' => $buf, 'host' => $from, 'port' => $port]);
            ...
            $this->logWriter('debug', ['udp first sending len' => $w_ret, 'udp first sending data' => '']);
            ...
            $this->logWriter('debug', [__METHOD__ => 'udp select', 'connection id' => $des['connection_id']]);
        }
    ...
    public function receiving(string $p_cid)
    {
        ...
        if($udp === true)
        {
            ...
            $this->logWriter('debug', [__METHOD__ => 'socket_recvfrom', "recv data" => $buf, 'host' => $from, 'port' => $port, 'connection id' => $p_cid]);
        }
        else
        {
            $rcv = '';
            ...
            $this->logWriter('debug', [__METHOD__ => 'socket_read', "recv data[{$rcv_siz}]" => $w_ret, 'connection id' => $p_cid]);
        }
    ...
    public function recv(string $p_cid, &$p_recv, int $p_size = null)
    {
        ...
        else
        {
            ...
            $this->logWriter('debug', [__METHOD__ => 'socket_read', "recv data" => $w_ret, 'connection id' => $p_cid]);
        }
    ...
    public function sending(string $p_cid): ?bool
    {
        ...
        if($udp === true)
        {
            ...
            $this->logWriter('debug', [__METHOD__ => 'socket_sendto', "send data" => $dat, 'size' => strlen($dat), 'sent size' => $w_ret, 'connection id' => $p_cid]);
        }
        else
        {
            // データ送信
            ...
            $this->logWriter('debug', [__METHOD__ => 'socket_write', "send data" => $dat, 'size' => strlen($dat), 'sent size' => $w_ret, 'connection id' => $p_cid]);
        }
